...
resource "google_compute_router_peer" "cloud_router_peers_region2" {
  count = "${var.vpc_router_count}"
  name                      = "${var.vpc_router_peer2_name}-${count.index}"
  router                    = "${var.vpc_router_name}-region2-${count.index}"
  peer_ip_address           = "${var.vpc_router_peer2_ip_address}"
  peer_asn                  = "${var.vpc_router_peer2_asn}"
  advertised_route_priority = "${var.vpc_router_peer2_advertised_route_priority}"
  interface                 = "interface-1"
